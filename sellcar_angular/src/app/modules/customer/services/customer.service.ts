...
export class CustomerService {
  ...
  getBidsByCarId(carId: number): Observable<any> {
    return this.http.get(BASE_URL + `api/customer/car/${carId}/bids`, { // Removed the unnecessary `}`
      headers: this.createAuthorizationHeader()
    });
}
  ...
  getAnalytics(): Observable<any> {
    return this.http.get(BASE_URL + `api/customer/car/analytics/${StorageService.getUserId()}`, { // Removed the unnecessary `}`
      headers: this.createAuthorizationHeader()
    });
}
